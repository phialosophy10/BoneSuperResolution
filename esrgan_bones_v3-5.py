## Ver. 3.5: Does inference on full slices instead of patches.
## Changed dataloader of slices to load full volume and then select slices from there
...
if log_wandb:
    DT = datetime.now()
    wandb.run.name = "train" + str(len(femur_no)) + "_" + str(n_epochs) + "epochs_" + DT.strftime("%d/%m_%H:%M")
...
for i in range(len(femur_no)):
    hr_path_train = dataset_path + "femur_" + femur_no[i] + "/micro/patches/f_" + femur_no[i] + "_???0*.*.npy"
    lr_path_train = dataset_path + "femur_" + femur_no[i] + "/clinical/patches/f_" + femur_no[i] + "_???0*.*.npy"
    img_paths_train.append([hr_path_train, lr_path_train])

hr_path_test = dataset_path + "femur_" + femur_no_test + "/micro/volume/f_" + femur_no_test + "_centered.npy"
lr_path_test = dataset_path + "femur_" + femur_no_test + "/clinical/volume/f_" + femur_no_test + "_centered.npy"
img_paths_test = [[hr_path_test, lr_path_test]]

# %% Train and test loaders
train_dataloader = DataLoader(PatchData(img_paths_train, hr_shape=hr_shape), batch_size=batch_size, drop_last=True, shuffle=True, num_workers=4)
test_dataloader = DataLoader(SliceData(img_paths_test), batch_size=batch_size_test, drop_last=False, shuffle=True, num_workers=4)
            ...
            if batch_idx+1 == len(test_dataloader)-1: 
                ...
                if epoch == n_epochs-1:
                    ...
                    for i in range(len(imgs_lr)):
                        ...
                        np.save("/work3/soeba/HALOS/Results/SR/" + slice_ids[i][:3] + "/slices/f_" + slice_ids[i], im_np)
                        im_uint8 = np.uint8(im_np * 255)
                        cv2.imwrite("/work3/soeba/HALOS/Results/SR/" + slice_ids[i][:3] + "/slices/f_" + slice_ids[i] + ".png", im_uint8)
