## Ver. 3.3: Removed the option to output as thickness image
## Also included the possibility to change between generator architectures
# %% Packages
import numpy as np
import pandas as pd
import os, math, sys
...
for i in range(len(femur_no)):
    hr_paths_train += sorted(glob.glob(dataset_path + "femur_" + femur_no[i] + "/micro/patches/f_" + femur_no[i] + "_???0*.*")) #added the "_???0" to only include every 10'th slice
    #hr_paths_train += sorted(glob.glob(dataset_path + "femur_" + femur_no[i] + "/micro/patches/f_" + femur_no[i] + "_???5*.*"))
    lr_paths_train += sorted(glob.glob(dataset_path + "femur_" + femur_no[i] + "/clinical/patches/f_" + femur_no[i] + "_???0*.*"))
    #lr_paths_train += sorted(glob.glob(dataset_path + "femur_" + femur_no[i] + "/clinical/patches/f_" + femur_no[i] + "_???5*.*"))

hr_paths_test = sorted(glob.glob(dataset_path + "femur_" + femur_no_test + "/micro/patches/f_" + femur_no_test + "*.*"))
lr_paths_test = sorted(glob.glob(dataset_path + "femur_" + femur_no_test + "/clinical/patches/f_" + femur_no_test + "*.*"))
...
for epoch in range(n_epochs):
    ...
    for batch_idx, imgs in enumerate(train_dataloader):
        ...
        if batches_done < warmup_batches:
            # Warm-up
            loss_G = lambda_pixel * loss_pixel + loss_content
            loss_G.backward()
            optimizer_G.step()
        ...
        loss_G = lambda_pixel * loss_pixel + loss_content + lambda_adv * loss_GAN
