## Ver. 3.8: Using new patch dataloader
...
import utils
import SR_config
from datasets import SliceData, PatchData_v2
...
log_wandb = True
if log_wandb:
    wandb.init(project="esrgan_bones_3.7_ucsf", entity="phialosophy", config={})
    DT = datetime.now()
    wandb.run.name = "train" + str(len(bone_no)) + "_" + str(args.n_epochs) + "epochs_" + DT.strftime("%d/%m_%H:%M")
    wandb.config.update({
...
for i in range(len(bone_no)):
    hr_paths_train.append(dataset_path + bone_no[i] + "/mct/vol/" + bone_no[i] + ".npy")
    if args.data_type == "real":
        lr_paths_train.append(dataset_path + bone_no[i] + "/XCT/vol/" + bone_no[i] + ".npy")
    else:
        lr_paths_train.append(dataset_path + bone_no[i] + "/synth/vol/" + bone_no[i] + ".npy")
    mask_paths_train.append(dataset_path + bone_no[i] + "/mask/vol/" + bone_no[i] + ".npy")
img_paths_train = []
...
for i in range(len(bone_no_test)):
    hr_paths_test.append(dataset_path + bone_no_test[i] + "/mct/vol/" + bone_no_test[i] + "_padded.npy")
    if args.data_type == "real":
        lr_paths_test.append(dataset_path + bone_no_test[i] + "/XCT/vol/" + bone_no_test[i] + "_padded.npy")
    else:
        lr_paths_test.append(dataset_path + bone_no_test[i] + "/synth/vol/" + bone_no_test[i] + "_padded.npy")
img_paths_test = []
for i in range(len(hr_paths_test)):
    img_paths_test.append([hr_paths_test[i], lr_paths_test[i]])
print(f'Dataloading took {(time() - dataload_start)} seconds')

# %% Datasets and dataloaders
num_samp_test = 0
for i in range(len(bone_no_test)):
    num_samp_test += SR_config.VOL_SHAPES_PADDED_UCSF[bone_no_test[i]][2]
rand_inds = random.sample(range(num_samp_test),SR_config.BATCH_SIZE_TEST)
test_sampler = SubsetRandomSampler(rand_inds)

train_dataloader = DataLoader(PatchData_v2(file_paths=img_paths_train, patch_size=SR_config.PATCH_SIZE), batch_size=args.batch_size, drop_last=True, shuffle=True, num_workers=4)
test_dataloader = DataLoader(SliceData(img_paths_test), batch_size=SR_config.BATCH_SIZE_TEST, drop_last=True, shuffle=False, sampler=test_sampler, num_workers=4)
...
for epoch in range(args.n_epochs):
    ...
    if save_models:
        torch.save(generator.state_dict(), "/dtu/3d-imaging-center/projects/2022_QIM_52_Bone/analysis/UCSF_data/Results/model/" + config + "/model.pt")
        print('Saved model.')
...
# Save model and training configs in designated folder
if save_models:
    with open("/dtu/3d-imaging-center/projects/2022_QIM_52_Bone/analysis/UCSF_data/Results/model/" + config + "/latest_run.txt", "w") as f:
