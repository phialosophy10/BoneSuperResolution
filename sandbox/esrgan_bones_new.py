...
if log_wandb:
    wandb.init(project="esrgan_bones_new", entity="phialosophy", config={})
...
class ImageDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        slice_name = slice_name[-15:-4] # Remove ".npy"
        #slice_id = slice_id[-12:-4]
        #femur_name = slice_name[:3]         #e.g. "01a"
        #slice_id = int(slice_name[-11:-7])  #e.g. 0294
        #patch_no = int(slice_name[-6:-4])   #e.g. 12
    ...
    for batch_idx, imgs in enumerate(test_dataloader):
        ...
        if epoch == n_epochs-1:
            ...
            for i in range(len(imgs_lr)):
                ...
                np.save(dataset_path + "/SR" + in_type[8:] + "/" + slice_ids[i], im_np)
                #im = Image.fromarray(np.uint8(im_np*255)).convert("L")
