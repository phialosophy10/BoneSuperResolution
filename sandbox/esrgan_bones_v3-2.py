## Ver. 3.2: Removed relativistic discriminator for regular discriminator
...
generator = ESRGenerator(channels=1, filters=64, num_res_blocks=4)         #num_res_blocks=16
    ...
    for batch_idx, imgs in enumerate(train_dataloader):
        ...
        with torch.cuda.amp.autocast(dtype=torch.float16):
            ...
            if batches_done < warmup_batches:
                ...
                if (batch_idx+1) % 10 == 0:
                    if log_wandb:
                        wandb.log({"loss/gen_loss": gen_loss/loss_count})
                loss_count += 1
                continue
            ...
            loss_G = lambda_pixel * loss_pixel + lambda_adv * loss_GAN + loss_content
