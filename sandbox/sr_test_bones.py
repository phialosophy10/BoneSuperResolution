...
n_epochs = 2
# name of the dataset
dataset_path = "/work3/soeba/HALOS/Data/Images/test"
# size of the batches
batch_size = 8 #16
# adam: learning rate
lr = 0.00008
...
hr_paths = sorted(glob.glob(dataset_path + "/hr/*.*"))
lr_paths = sorted(glob.glob(dataset_path + "/lr/*.*"))

img_paths = []
for i in range(len(hr_paths)):
    img_paths.append([hr_paths[i], lr_paths[i]])
...
train_paths, test_paths = train_test_split(img_paths, test_size=0.02, random_state=42)
...
for epoch in range(n_epochs):
    ...
    if np.argmin(test_gen_losses) == len(test_gen_losses)-1:
        torch.save(generator.state_dict(), "/work3/soeba/HALOS/Data/celebA/saved_models/generator.pth")
        torch.save(discriminator.state_dict(), "/work3/soeba/HALOS/Data/celebA/saved_models/discriminator.pth")
