...
if log_wandb:
    wandb.init(project="srresnet_bones", entity="phialosophy", config={})

# Variable settings
femur_no = ["01","15","21"] #"01", "15", "21", "74"                    # choose which bone to train on
...
for epoch in range(n_epochs):
    ...
    for batch_idx, imgs in enumerate(train_dataloader):
        ...
        loss_image = criterion_GAN(imgs_hr, gen_hr)
        # Total loss
        loss_G = alpha * loss_content + loss_image
        loss_G.backward()
        optimizer_G.step()
        ...
        if batch_idx == len(train_dataloader)-1: 
            print(f'Trained batch {batch_idx} in epoch {epoch}, gen_loss = {gen_loss/(batch_idx+1)} \n')
        if log_wandb:
            wandb.log({"loss/train_loss": gen_loss/(batch_idx+1)})
    ...
    for batch_idx, imgs in enumerate(test_dataloader):
        ...
        if log_wandb:
            wandb.log({"loss/test_loss": gen_loss/(batch_idx+1)})
