...
# Style loss code found here: https://github.com/Octaves0911/Neural_Style_Transfer/blob/master/NST.py
...
# Variable settings
femur_no = ["01","21","74"] #"01", "15", "21", "74"                    # choose which bone to train on
femur_no_test = "15"                                       # choose which bone to test on 
...
if log_wandb:
    ...
    "input_type": in_type,
    "loss_coefficient_GAN": coeff_GAN,
    "loss_coefficient_style": coeff_style,
...
def calc_style_loss(gen,style):
    ...
    G=torch.mm(gen.view(channel,height*width*batch_size),gen.view(channel,height*width*batch_size).t())
    A=torch.mm(style.view(channel,height*width*batch_size),style.view(channel,height*width*batch_size).t())
...
for epoch in range(n_epochs):
    ...
    for batch_idx, imgs in enumerate(train_dataloader):
        ...
        loss_style = calc_style_loss(gen_hr,imgs_hr)
        # Total loss
        loss_G = coeff_style * loss_style + loss_content + coeff_GAN * loss_GAN #loss_content + 1e-3 * loss_GAN
        loss_G.backward()
        optimizer_G.step()
