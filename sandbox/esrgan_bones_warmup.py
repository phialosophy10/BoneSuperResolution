...
if log_wandb:
    wandb.init(project="esrgan_bones_warmup", entity="phialosophy", config={})
...
batch_size = 8 #8
lr = 0.00008
warmup_epochs = 10                                           # number of epochs to run before starting training of discriminator
...
test_dataloader = DataLoader(ImageDataset(img_paths_test, hr_shape=hr_shape), batch_size=int(batch_size*0.75), drop_last=True, shuffle=False, num_workers=1)
...
for epoch in range(n_epochs):
    ...
    for batch_idx, imgs in enumerate(train_dataloader):
        ...
        loss_ims = criterion_content(imgs_hr,gen_hr)
        # Total loss
        if epoch >= warmup_epochs:
            loss_G = loss_content + coeff * loss_GAN #loss_content + 1e-3 * loss_GAN
        else:
            loss_G = 0.5 * loss_content + loss_ims
        loss_G.backward()
        optimizer_G.step()
    ...
    for batch_idx, imgs in enumerate(test_dataloader):
        ...
        if batch_idx+20 == len(test_dataloader)-1: 
            ...
            #         im_name = img_paths_test[batch_idx*len(imgs)+i][0]
            #         im.save(dataset_path + "/SR/" + in_type[8:] + "/" + im_name[-11:-4] + ".jpg")
