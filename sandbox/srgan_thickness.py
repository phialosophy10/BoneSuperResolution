...
if log_wandb:
    wandb.init(project="srgan_thickness", entity="phialosophy", config={})

# Variable settings
femur_no = "21" #"01", "15", "21", "74"                    # choose which bone to train on
femur_no_test = "21"                                       # choose which bone to test on 
in_type = "micro" #"clinical", "micro"                     # choose whether input in clinical CT or micro CT
out_type = "thick" #"micro", "thick"                       # choose whether target is micro CT or thickness image
if in_type == "clinical":
...
#     def __init__(self, in_channels=1, out_channels=1, n_residual_blocks=24): #16
...
generator = utils.make_monai_net(kernel=kernel_size)
...
criterion_dice = monai.losses.DiceLoss(sigmoid=True, to_onehot_y=False)
...
for epoch in range(n_epochs):
    ...
    for batch_idx, imgs in enumerate(train_dataloader):
       ...
       # loss_content = criterion_content(gen_features, real_features.detach())
        loss_content = criterion_dice(imgs_hr,gen_hr)
        # Total loss
