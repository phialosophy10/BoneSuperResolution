...
if log_wandb:
    wandb.init(project="esrgan_bones", entity="phialosophy", config={})
...
test_dataloader = DataLoader(ImageDataset(img_paths_test, hr_shape=hr_shape), batch_size=batch_size, drop_last=False, shuffle=False, num_workers=1)
...
for epoch in range(n_epochs):
    ...
    for batch_idx, imgs in enumerate(test_dataloader):
        ...
        if epoch == n_epochs-1:
            #print(f'saving images from batch no {batch_idx} out of {len(test_dataloader)} total batches')
            generator.eval()
...
print(f'Saved {saved_im_count} images out of {len(lr_paths_test)} test images')